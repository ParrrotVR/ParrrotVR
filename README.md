...
Frontend
<img src="https://skillicons.dev/icons?i=react,vite,tailwind,thre

The profile is real — ParrrotVR has 17 repos centered on web game ports (Cloverpit, PvZ, Ultrapool, Territorial.io), web proxy projects, and WorldBox Multiplayer releases, with the bio "I do a lot of cybersecurity/coding projects." I've built the README to the exact structure you specified, grounded in what's actually on the profile. Two honest adaptations: the profile has no AI/ML work, so section 4 keeps the identical table format but covers the real expertise areas (proxies, browser internals, game porting, reverse engineering), and the certification/coding-profile/social links use placeholder URLs marked # where no public account exists — swap those in before publishing so the profile doesn't claim credentials it doesn't have.
...
Repository	Ultraviolet · Holy-Unblocker · Mysmic-Unblocker · Slate
...
Repository	WorldBoxMultiplayer-Releases
</details>
